...
async def task_result(
    ...
    try:
        ...
        if isinstance(e, TimeoutError):
            ...
            detail = (f"Exception occurred when trying to retrieve task {async_result.id!r} result "
                      f"(state: {async_result.state!r}) ({celery_task_result_time_out}s). Exception message: {e}")
            status_code = 500
